"""그래프 상태의 proposals 필드용 병합. 순수 함수.

proposals 는 {구간 idx: SequenceItem}. 두 가지 병합이 있음.
- merge_proposals: LangGraph 리듀서. Send 로 병렬 실행된 노드들의 결과를 idx 단위로 합친다.
- apply_regeneration: 부분 재생성. 피드백 라우팅이 지정한 targets 밖의 구간은 바이트 단위로 그대로.
"""
...
def merge_proposals(left: Proposals | None, right: Proposals | None) -> Proposals:
    """LangGraph 리듀서. 같은 idx 는 right 가 이기며 입력은 수정하지 않음."""
    return {**(left or {}), **(right or {})}


def apply_regeneration(existing: Proposals, regenerated: Proposals, targets: set[int]) -> Proposals:
    """targets 구간만 regenerated 로 교체한 새 dict 를 돌려줌. 나머지 구간은 같은 객체를 유지.
